...
//!
//! These tests require a running Supabase instance with tables created.
//! Set SUPABASE_URL and SUPABASE_ANON_KEY environment variables to run these tests.
//!
//! Run with: cargo test -p zoey-storage-supabase --test supabase_integration_tests -- --ignored

use zoey_core::types::*;
use zoey_storage_supabase::{SupabaseAdapter, supabase::SupabaseConfig};

async fn setup_adapter() -> Option<SupabaseAdapter> {
...
async fn test_agent_crud() {
    let Some(adapter) = setup_adapter().await else {
        eprintln!("Skipping test - Supabase not available");
        return;
    };
...
async fn test_memory_crud() {
    ...
    let agent = Agent {
        ...
        character: serde_json::json!({"name": "TestAgent"}),
        created_at: Some(chrono::Utc::now().timestamp()),
        updated_at: None,
    };
    adapter.create_agent(&agent).await.unwrap();
    ...
    adapter.create_entities(vec![entity]).await.unwrap();
    ...
    adapter.create_room(&room).await.unwrap();
...
async fn test_entity_operations() {
    ...
    let agent = Agent {
        id: agent_id,
        name: "Entity Test Agent".to_string(),
        character: serde_json::json!({"name": "TestAgent"}),
        created_at: Some(chrono::Utc::now().timestamp()),
        updated_at: None,
    };
...
async fn test_world_and_room() {
    ...
    let agent = Agent {
        id: agent_id,
        name: "World Test Agent".to_string(),
        character: serde_json::json!({"name": "TestAgent"}),
        created_at: Some(chrono::Utc::now().timestamp()),
        updated_at: None,
    };
...
async fn test_participants() {
    ...
    let agent = Agent {
        id: agent_id,
        name: "Participant Test Agent".to_string(),
        character: serde_json::json!({"name": "TestAgent"}),
        created_at: Some(chrono::Utc::now().timestamp()),
        updated_at: None,
    };
...
async fn test_tasks() {
    ...
    let agent = Agent {
        id: agent_id,
        name: "Task Test Agent".to_string(),
        character: serde_json::json!({"name": "TestAgent"}),
        created_at: Some(chrono::Utc::now().timestamp()),
        updated_at: None,
    };
    ...
    // Pending tasks should now be empty (for this task)
    let pending = adapter.get_pending_tasks(agent_id).await.unwrap();
    assert!(pending.iter().all(|t| t.id != task_id));
...
async fn test_connection() {
    ...
    let is_ready = adapter.is_ready().await.unwrap();
    assert!(is_ready);
}
